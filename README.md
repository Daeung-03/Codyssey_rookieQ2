...
### 1. 퀴즈 게임 개발
1. 객체지향 및 레이어드 아키텍쳐를 도입하여 구조를 셋팅합니다. [x]

  1.1 핵심은 모델과 서비스 계층을 구분하는 것입니다. [x]

  1.2 model(클래스 정의) -> repository(json I/O) -> services(게임 로직, 저장 로직) -> ui(터미널 입출력) [x]

  1.3 레이어에 맞게 디렉토리 구조 설계 [x]

2. 독립된 패키치 설치공간을 제공하는 파이썬의 가상환경을 사용합니다. 또한 사용된 라이브러기 발생한다면 requirments.txt로 저장합니다. [x]

  2.1 venv 활성화, 파이썬 버전 선택 [x]

  2.2 pip3 freeze 사용하여 requirments.txt 생성 [x]

3. .gitignore를 작성합니다. 기초는 gitignore.io에서 가져옵니다. [x]

4. 클래스 작성 [x]

  4.1 quiz 클래스 구현(속성: 문제, 선지, 정답 / 메소드: 퀴즈 출력, 정답 확인) [x]

  4.2 quiz_game 클래스 구현(속성: 최대 정답 수, 퀴즈 목록(List) / 메소드: 메뉴 표시, 퀴즈 풀기, 퀴즈 추가, 목록 보기, 점수 확인, 파일 저장/불러오기) [x]

5. 서비스 구현 [x]

  5.1 기본 퀴즈 데이터 추가, 풀기 기능 구현 [x]

  5.2 퀴즈 목록 보기, 추가 기능 구현 [x]

  5.3 점수 저장 및 확인 기능 구현 [x]

  5.4 Json 입출력 구현 [x]

  5.5 quiz_game 클래스 구조 통합 [x]
...
**1. 캡슐화 및 브랜치 관리의 아쉬움**
파일 및 세부 구현 단위로 브랜치를 쪼개서 작업하다 보니, 단순한 변수명 규칙 수정(`max_score` -> `best_score`)에도 여러 연관 브랜치에서 3번 이상의 커밋과 푸시가 연쇄적으로 발생했습니다. 이는 특정 객체의 속성이 외부에 강하게 노출되어 의존성이 높았기 때문이기도 합니다. 차라리 독립적인 `fix` 브랜치를 운용했어야 했으며, 동시에 정보 은닉과 캡슐화라는 객체지향(OOP) 원칙을 더 철저히 지켜야 함을 체감했습니다.

**2. 레이어 간 강한 의존성**
레이어드 아키텍처를 도입해 패키지와 코드를 구조적으로 분리했음에도, 작은 기능 하나를 추가할 때마다 여러 계층(Model, Service, UI 등)의 파일을 동시에 수정해야 했습니다. 결과적으로 레이어 간 결합도가 여전히 높다는 것을 확인했으며, 단일 책임 원칙(SRP)과 같은 객체지향 파괴 없이 온전하게 역할이 분배된 것인지에 대한 고민이 남았습니다. 
...
### `state.json` 생성 및 기본 사용법

프로그램을 최초로 실행(`python3 main.py`)하면 기본 퀴즈 데이터가 포함된 `state.json` 파일이 루트 디렉토리에 자동으로 생성됩니다. 
만약 수동으로 초기 데이터를 구성하거나 커스텀 퀴즈를 직접 추가하고 싶다면, 프로젝트 루트 경로에 `state.json` 파일을 생성하고 아래와 같은 형식으로 작성할 수 있습니다.
